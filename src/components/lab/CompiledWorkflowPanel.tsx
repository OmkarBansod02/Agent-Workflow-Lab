    ...
    <div className="rounded-xl border-l-2 border-l-[#FF5A2A]/60 border border-white/[0.08] bg-[#1B1A18] p-5 space-y-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2.5">
          <h2 className="text-sm font-semibold text-stone-100">Compiled Workflow</h2>
      ...
      <div className="space-y-2">
        <span className="text-[10px] font-medium font-mono text-[#78716C] uppercase tracking-wider">Tools</span>
      ...
      {workflow.generatedSearchQueries.length > 0 && (
        <div className="space-y-2">
          <span className="text-[10px] font-medium font-mono text-[#78716C] uppercase tracking-wider">
            Generated Search Queries
          </span>
            ...
            {workflow.generatedSearchQueries.map((q) => (
              <div key={q.id} className="flex items-start gap-2 rounded-md bg-[#201F1D] border border-white/[0.06] px-3 py-2">
                ...
                <div className="min-w-0">
                  <code className="text-xs font-mono text-stone-300">{q.query}</code>
                  <p className="text-[10px] text-[#78716C] mt-0.5">{q.purpose}</p>
                </div>
              </div>
            ))}
          </div>
            ...
            {workflow.approvalGates.map((gate) => (
              <div key={gate.id} className="flex items-start gap-2 rounded-md border-l-2 border-l-amber-400 bg-amber-500/10 border border-amber-500/20 px-3 py-2">
                <div className="min-w-0">
                  ...
                  <p className="text-xs text-[#A8A29E] mt-0.5">{gate.reason}</p>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="space-y-2">
          ...
          <ul className="space-y-1">
            {workflow.missingInfo.map((info, i) => (
              <li key={i} className="text-xs text-[#A8A29E] flex items-start gap-2">
              ...
              >
                ...
                <div className="space-y-1 min-w-0">
                  <div className="flex items-center gap-1.5 flex-wrap">
                    <span className="text-xs font-medium text-stone-200">{step.title}</span>
...
function Field({ label, value }: { label: string; value: string }) {
  return (
    <div className="space-y-0.5">
      <span className="text-[10px] font-medium font-mono text-[#78716C] uppercase tracking-wider">{label}</span>
